...
void CasoDePrueba::CheckHayCamino(Puntero<Sistema> interfaz, Puntero<Grafo<Cadena, int>> grafo, Cadena origen, Cadena destino, bool esperado) {
	...
	if (esperado == interfaz->HayCamino(grafo, origen, destino)) {
...
void CasoDePrueba::Prueba4TADGrafo() {
	...
	if (grafo != nullptr) {
		...
		Verificar(interfaz->EsConexo(grafo), DEBILMENTE_CONEXO, mensaje);

		grafo->AgregarArco(ciudades[1], ciudades[4], 10);
		grafo->AgregarArco(ciudades[4], ciudades[0], 10);

		Verificar(interfaz->EsConexo(grafo), FUERTEMENTE_CONEXO, mensaje);
		...
		Verificar(interfaz->EsConexo(grafo), NO_CONEXO, mensaje);
	}
	CerrarSeccion();
}
...
void CasoDePrueba::Prueba5TADGrafo() {
	...
	if (grafo != nullptr) {
		...
		auto f = [](int i)->Costo {
			return *new Costo(Tupla<int, int, int, int>(i, 0, 0, 0));
		};
		obtenido = interfaz->ArbolCubrimientoMinimo(grafo, f);
...
void CasoDePrueba::Prueba6TADGrafo() {
	...
	if (grafo != nullptr) {
		...
		Iterador<Iterador<Cadena>> obtenido = interfaz->ComponentesConexas(grafo, f);
